...
import ABI from "../../lib/contracts/POLYGON/ABI1.json";
import usdcABI from "../../lib/contracts/USDC_ABI.json";
	...
	async function getData() {
		if($web3 != undefined) {
			...
			if($chainId == 137) {
				...
				contract2 = new $web3.eth.Contract(usdcABI, usdcAddress);
				let allowance = await contract2.methods.allowance($selectedAccount, CONTRACT).call({ from: $selectedAccount });
		...
		<section class="section2">
			...
			<!-- <div class="cover"></div> -->
				<div>
					...
					<h2>{parseInt((prize.toString()).slice(1, (prize.toString()).length -5)).toLocaleString('en')} $USDC</h2>
				</div>
				...
				<div class="content-container">
					...
					<h2>10,000 $USDC</h2>
					<br> -->
					<p>This competition has a maximum of {(totalTickets.toLocaleString('en'))} entries.</p>
					...
					<div class="ticket-amount">
						...
						<span class="right">{(totalTickets).toLocaleString('en')}</span>
						...
						{/if}
						{#if soldTickets < 500}
						<p>{(remainingTickets).toLocaleString('en')} remaining!</p>
						{/if}
					</div>
			...
			<div class="info-box-container">
				<div class="info-box">
					...
					<div>
						...
						<p>1:{(totalTickets).toLocaleString('en')}</p>
					</div>
				</div>
				<div class="info-box">
					...
					<div>
						<h2>Prize Pool</h2>
						<p>${((prize.toString()).slice(1, prize.length -5)).toLocaleString('en')}</p>
					</div>
				</div>
				<div class="info-box">
